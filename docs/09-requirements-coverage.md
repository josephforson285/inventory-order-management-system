...
| Record every stock change in an inventory log | `trg_products_after_update` — a **trigger**, so no write path can bypass it (rule I-02) | ○ |
...
| Full history retrievable for auditing | Append-only enforced by immutability triggers and revoked `UPDATE`/`DELETE` grants (rule I-03) | ○ |
...
| Ensure the inventory log reflects replenishment | The same `trg_products_after_update` trigger, logging `REPLENISHMENT` | ○ |
| Automate stock updates after an order | Trigger-driven; the procedure never writes the log itself | ○ |
| Automate total amount calculation | `trg_order_details_after_insert` + the generated `net_amount` | ○ |
| Automate customer tier categorisation | `trg_orders_after_insert` / `trg_orders_after_update` maintain `customers.tier_id` (rule T-05) | ○ |
